...
rules_texts = case rand(1..100)
              when 1..85  then common_rules
              when 86..99 then common_rules[0..1].unshift(rare_rule)
              when 100    then common_rules[0..1].unshift(epic_rule)
              end
...
#post to twitter
media_ids = output_paths.map do |filename|
 twitter.upload(File.new(filename))
end

twitter.update("New cards: #{words.join(', ')}", media_ids: media_ids.join(','))
